...
function ssg_show_filter_Callback($matches) 
{
	...
	$ssg_xml = $ssg_xml .'var mygallery=new SuperbSlideshowGallery({wrapperid: "'.$newwrapperid.'", dimensions: ['.$ssg_width.', '.$ssg_height.'], imagearray: [ '.$ssg_package.' ],displaymode: {type:"'.$type.'", pause:'.$ssg_pause.', cycles:'.$ssg_cycles.', wraparound:false},	persist: false, fadeduration: '.$ssg_fadeduration.', descreveal: "always",togglerid: ""})';
...
function ssg_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_ssg_TABLE . "'") != WP_ssg_TABLE) 
	{
		...
		$sSql = $sSql . "`ssg_title` VARCHAR( 200 ) NOT NULL ,";
...
function ssg_admin_option() 
{
	...
	echo '<p>Description option :<br><input  style="width: 100px;" type="text" value="';
	echo $ssg_descreveal . '" name="ssg_descreveal" id="ssg_descreveal" /> (ondemand/always)</p>';
